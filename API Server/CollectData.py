...
def receive_request():
    if request.method == 'POST':
        ...
        if 'key' in data:
            value = data['key']  # Access the value associated with the 'key' key
            # Process the received data here
            return data
        else:
            return 'Key "key" not found in the received data', 400  # Return a 400 Bad Request error if 'key' is not found
